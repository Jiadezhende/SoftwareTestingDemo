...
| v1.1 | 2026-04-21 | 成员B | 根据 BlackBoxSelfCheck 补充状态机缺口的 @Disabled 自查项 |
...
| UT-OR-018 | 输入状态为 `STATE_WAIT / STATE_FINISH / STATE_REJECT` 的订单 | 应拒绝非法前置状态并抛出业务异常，不更新订单状态 | 实际未拦截非法状态，直接更新为 `STATE_WAIT` | 错误 |
...
| UT-OR-019 | 输入状态为 `STATE_NO_AUDIT / STATE_REJECT` 的订单 | 应拒绝非法前置状态并抛出业务异常，不更新订单状态 | 实际未拦截非法状态，直接更新为 `STATE_FINISH` | 错误 |
...
| UT-OR-020 | 输入状态为 `STATE_WAIT / STATE_FINISH` 的订单 | 应拒绝非法前置状态并抛出业务异常，不更新订单状态 | 实际未拦截非法状态，直接更新为 `STATE_REJECT` | 错误 |
